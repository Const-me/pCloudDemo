...
namespace PCloud
{
	/// <summary>Parses pCloud's proprietary binary JSON into untyped .NET objects.</summary>
	...
	class ResponseParser
	{
		...
		{
			// Too bad there're no algebraic types in current C#, boxing all these numbers is relatively inefficient. OTOH, these dictionaries only contain a couple simple keys/values for my use case, no big deal.
			while( true )
			{
		...
		object parseValue()
		{
			...
			switch( tp )
			{
				...
				case 19:
					return true;
				case 20:
					return new Response.Data( (long)readInt8() );
			}
